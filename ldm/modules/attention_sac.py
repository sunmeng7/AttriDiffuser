...
class GatedSelfAttentionDense(nn.Module):
    def __init__(self, query_dim, context_dim,  n_heads, d_head):
        ...
        # 可学习参数 将一个不可训练的tensor转换成可以训练的类型parameter, 并将这个parameter绑定到这个module里面
    ...
    def forward(self, x, opts):
        ...
        # x = x + self.scale*torch.tanh(self.alpha_attn) * self.attn(self.norm1(torch.cat([x,opts],dim=1)), opts, opts)[:,0:N_visual,:]
