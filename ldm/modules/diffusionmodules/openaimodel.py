...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb, context=None, opts=None):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb)
            elif isinstance(layer, SpatialTransformer):
                x = layer(x, context, opts=opts)
            else:
                x = layer(x)
        return x
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.grounding_tokenizer = grounding_tokenizer
    ...
    def forward(self, x, timesteps=None, opts=None, context=None, y=None, **kwargs):
        """
        ...
        for module in self.input_blocks:
            # h = module(h, emb, context)     # 图片,时间步,文本
            h = module(h, emb, context, opts)  # 图片,时间步,文本, 属性
            hs.append(h)
        # h = self.middle_block(h, emb, context)
        h = self.middle_block(h, emb, context, opts)
        for module in self.output_blocks:
            h = th.cat([h, hs.pop()], dim=1)
            # h = module(h, emb, context)
            h = module(h, emb, context, opts)
        h = h.type(x.dtype)
        if self.predict_codebook_ids:
