...
class CrossAttention(nn.Module):
    ...
    def __init__(self, query_dim, context_dim=1024, heads=8, dim_head=64, dropout=0.):
        super().__init__()
...
class GatedSelfAttentionDense(nn.Module):
    ...
    def forward(self, x, opts):
        ...
        # assert x.shape[0]==opts.shape[0]
        x = x + self.scale * torch.tanh(self.alpha_attn) * self.attn(self.norm1(torch.cat([x, opts], dim=1)),cross_opts, cross_opts)[:, 0:N_visual, :]
        # temp1 = self.attn(self.norm1(torch.cat([x, opts], dim=1)), cross_opts, cross_opts)[:, 0:N_visual, :]
        # temp2 = self.scale * torch.tanh(self.alpha_attn) * temp1
        # x = x + temp2
...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, x, context=None, opts=None):
        ...
        # opts = rearrange(opts, 'b c h w -> b (c h) w')
