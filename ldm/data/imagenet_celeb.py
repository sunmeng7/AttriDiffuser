...
'''
    23-08-20
    celeba dataset used for DIFFUSION + discriminator
'''

class MutilVox(Dataset):
    root = '/home/sunmeng/datasets/'
    ...
    def __init__(
        ...
        splits = pd.read_csv(fn("celeba.csv"))     # 划分数据集的文件, train val test
        mask = slice(None) if split is None else (splits["split"] == split)

        pkl_file_prefix = "celeb"
        ...
        dis_emb = os.path.join(self.root, f"emb_{split_str}.pkl")
        with open(os.path.join(self.root, dis_emb), "rb") as fp:
            self.dis_emb_lst = pickle.load(fp)
    ...
    def __getitem__(self, i):
        ...
        disemb = self.dis_emb_lst[i]
        # tmp = []
        # tmp.append(disemb[0]['embedding'])
        # example["dis_embedding"] = tmp
        example["dis_embedding"] = torch.Tensor(disemb[0]['embedding']).unsqueeze(dim=0).cuda()
